...
const VotingSystem = () => {
  ...
  const handleCredentialResponse = async (response) => {
    try {
      ...
      // 🔐 Connexion côté Supabase avec le token Google
      try {
        ...
        if (authError) {
          ...
          setError(
            'Failed to authenticate with the backend. Please try again.'
          );
          return;
        }
      } catch (authErr) {
      ...
      try {
        ...
        window.google.accounts.id.prompt((notification) => {
          ...
          if (
            ...
            console.log(
              'Prompt was not displayed, reason:',
              notification.getNotDisplayedReason()
            );
            setError(
              'Please allow popups and ensure third-party cookies are enabled'
            );
          }
        });
      } catch (err) {
  ...
  // 🔁 Utilise la fonction SQL get_active_polls() pour charger sondages + options
  ...
  // 🗳 Vote avec les fonctions SQL has_user_voted + increment_vote + trigger anti-duplication
  const vote = async (pollId, optionId) => {
    ...
    try:
    {
      ...
      // Vérification côté BDD (fonction has_user_voted)
      ...
      // 1) Enregistrer le vote (table votes) -> trigger prevent_duplicate_votes protège
      ...
      if (voteError) {
        // Si le trigger renvoie "User has already voted on this poll"
        if (
          voteError.message &&
      ...
      // 2) Incrémenter le compteur côté poll_options via increment_vote()
      ...
      // Recharger les sondages pour mettre à jour les % / barres
      await loadPolls();
  ...
  // 🗑 Suppression via la fonction admin_delete_poll() (supprime options + votes + poll)
